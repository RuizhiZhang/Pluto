...
public class TicketMasterClient {
	...
	private static final String API_KEY = "rvnfgaX4VQGiGgmBMneqeQeU2CEVGlj5";
	...
	public List<Item> search(double lat, double lon, String keyword) {
		...
		try {
			// Create a URLConnection instance that represents a connection to the remote // object referred to by the URL. The HttpUrlConnection class allows us to
			...
			// can use the getResponseCode(), connect(), getInputStream() or // getOutputStream() methods.
			...
			BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream())); //InputStreamReader 会一口气把输入全读进来，但这样其实很危险，所以要BufferReader来搞成一行一行的读
			String line;
			while ((line = reader.readLine()) != null) { 
				responseBody.append(line);
			}
			reader.close(); //BufferReader记得关哦
